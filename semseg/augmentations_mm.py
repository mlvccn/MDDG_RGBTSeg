    ...
    def __call__(self, sample: list) -> list:
        for k, v in sample.items():
            if k == 'mask':
                ...
                sample[k] /= 255
                sample[k] = TF.normalize(sample[k], self.mean[0:3], self.std[0:3])
            else:
                sample[k] = sample[k].float()
                sample[k] /= 255
                # print(self.mean[3])
                sample[k] = TF.normalize(sample[k], [self.mean[3]], [self.std[3]])
...
class RandomGrayscale:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            sample['img'] =  TF.rgb_to_grayscale(sample['img'], 3)
        return sample
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        ...
        RandomAdjustSharpness(sharpness_factor=0.1, p=0.2),
        ...
        Normalize((0.485, 0.456, 0.406, 0.499), (0.229, 0.224, 0.225, 0.266))
    ])
